...
<template>
  ...
  <div class="text-center text-3xl mx-40">
      ...
      <div class="grid max-w-sm">
        ...
        <input required="true" maxlength="32" class="border-2 border-zinc-800 text-zinc-500 bg-zinc-900 rounded-sm focus:outline-none scale-[100%] ease-in duration-300 focus:scale-[102%]" id="first-name" placeholder="Sbans@sbans.com" v-model="email" />
      </div>
      <div class="grid max-w-sm pt-1">
        <Label for="last-name" class="select-none">Password</Label>
        <input maxlength="32" class="border-2 border-zinc-800 text-zinc-500 bg-zinc-900 rounded-sm focus:outline-none scale-[100%] ease-in duration-300 focus:scale-[102%]" id="last-name" placeholder="password123" required v-model="password" />
      </div>
      <div class="mt-2 mx-auto hover:bg-zinc-800 py-1 px-1 rounded-lg ease-in duration-300">
      <Button type="button" class="w-full disabled:opacity-40" @click="handleSignIn">Login</Button>
    </div>
    </div>
